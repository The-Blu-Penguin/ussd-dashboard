...
<template>
  <div v-if="show" class="w-80 border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-20 h-full">
    ...
    <div v-if="selectedNode" class="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-1">
      ...
      <div v-if="selectedNode.data?.componentType === 'ACTION'" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Endpoint URL (Optional)</label>
          <input 
            type="text" 
      ...
      <div v-if="selectedNode.data?.componentType === 'END'" class="space-y-4">
        <div>
          <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">End Message</label>
          <textarea 
            v-model="selectedNode.data.prompt"
            rows="3"
            class="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-vibes-500 focus:border-vibes-500 transition-colors resize-none"
